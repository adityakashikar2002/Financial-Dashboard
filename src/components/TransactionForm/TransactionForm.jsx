...
const TransactionForm = ({ transaction, isEdit, onSuccess }) => { // Accept onSuccess prop
  const navigate = useNavigate();
  const [formData, setFormData] = useState({
    transaction_name: transaction?.transaction_name || '',
    type: transaction?.type || 'debit',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // You can also add a success message here before navigating
      alert(`Transaction ${isEdit ? 'updated' : 'added'} successfully!`);

      // Navigation is now handled by the onSuccess callback in AddEditTransaction
      // navigate('/transactions'); // Remove or comment out this line
    } catch (err) {
      console.error('Error saving transaction:', err);
      setError('Failed to save transaction. Please try again.');
    } finally {
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-group">
          ...
          <input
            type="datetime-local"
            name="date"
            value={formData.date} // Use formData.date directly after initial formatting
            onChange={handleChange}
            required
          />
        </div>
